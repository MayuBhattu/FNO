...
for c_initial_value in c_initial_values:
    for J_dim_value in J_dim_values:
        sim_id = sim_id + 1
        ...
        J_bc = Constant(J_dim_value * L_dim / (D * c_max))
        ...
        while t < T + 1e-9:
            # Solve diffusion
            solve(F_diff == 0, c,  solver_parameters=diffusion_solver_params)
        ...
        # (2) Extract solution values at mesh nodes
            c_values = c.dat.data_ro[:]  # Concentration
            u_values = u.dat.data_ro[:, 0]  # Displacement in x
            v_values = u.dat.data_ro[:, 1]  # Displacement in y
        ...
        # #c_plot = axs[0].tricontourf(x, y, c_values, levels=50, cmap="plasma")
